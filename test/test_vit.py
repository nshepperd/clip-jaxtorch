...
def test_vit():
    ...
    jax_model = vit.VITB32() # vit.VisionTransformer(224, 32, 768, 12, heads=12, output_dim=512)
    params = jax_model.init_weights(rng.split())

    jax_model.load_state_dict(params, tree_map(lambda x: jnp.array(x.cpu().numpy()).astype(jnp.float32), clip_model.state_dict()))
    ...
    out_jax = norm1(jax_model.visual(Context(params, None), image))
    ...
    text = fromtorch(clip.tokenize("hello world"))
    out_jax = norm1(jax_model.encode_text(Context(params, None), text))
    out_torch = norm1(fromtorch(clip_model.encode_text(fromjax(text))))
    print((out_jax * out_torch).sum())
    print(jnp.sort((out_jax - out_torch).abs(), axis=1))
    assert jnp.allclose(out_torch, out_jax, atol=0.001, rtol=0.001)
...
def test_vitl14():
    ...
    jax_model = vit.VITL14() # vit.VisionTransformer(224, 32, 768, 12, heads=12, output_dim=512)
    params = jax_model.init_weights(rng.split())
    ...
    for (k, v) in clip_model.named_parameters():
        if k in params and params[k].shape == v.shape:
            ...
            print(f'{k} missing in jax model!')
            all_good = False
    assert all_good
